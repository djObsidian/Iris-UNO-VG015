...
#define SPI_Flag_RxFIFONotEmpty SPI_SR_RNE_Msk /*!< Флаг наличия данных в буфере приемника данных */
#define SPI_Flag_RxFIFOFull SPI_SR_RFF_Msk     /*!< Флаг заполнения буфера приемника */
#define SPI_Flag_TxFIFONotFull SPI_SR_TNF_Msk  /*!< Флаг наличия свободного места в буфере передатчика */
#define SPI_Flag_TxFIFOEmpty SPI_SR_TFE_Msk    /*!< Флаг пустоты буфера передатчика */
...
#define IS_SPI_SCK_DIV_EXTRA(VALUE) (((VALUE) > 1) && ((VALUE) < 0xFF) && (((VALUE) & 0x1) == 0))
...
/**
  ...
  *                  то результат соответствует логическому ИЛИ их состояний.
  */
...
/**
  ...
  * @param   SPIx  Выбор модуля SPI, где x=0|1
  * @param   FIFOLevelRx  Значение порога опустошения FIFO.
...
/**
  ...
  * @param   SPIx  Выбор модуля SPI, где x=0|1
  * @param   FIFOLevelTx  Значение порога опустошения FIFO.
  *                       Параметр принимает любое значение из диапазона 0-8.
  * @retval  void
  */
